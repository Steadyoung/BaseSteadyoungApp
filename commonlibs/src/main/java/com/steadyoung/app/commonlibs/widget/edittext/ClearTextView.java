package com.steadyoung.app.commonlibs.widget.edittext;
...
public class ClearTextView extends AppCompatTextView {
    ...
    public ClearTextView(final Context context) {
        super(context);
        init(context);
    }

    public ClearTextView(final Context context, final AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public ClearTextView(final Context context, final AttributeSet attrs, final int defStyleAttr) {
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        if(!isInit){
            LogUtils.d("ClearEditText还未初始化，无法启动动画");
            return;
        }
        if(text.length()>0) {
            if (!isVisible) {
